...
import com.example.aplicacionprueba.controller.PageController
...
class SecondFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        tabLayout =vista.findViewById(R.id.tabLayoutSF)
        viewPager = vista.findViewById(R.id.viewPagerSF)
        pageController = PageController(childFragmentManager, tabLayout.tabCount)
        viewPager.adapter=pageController
        tabLayout.addOnTabSelectedListener(object : OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                viewPager.currentItem = tab.position
                if (tab.position.equals(0))
                {
